...
public class Num implements Comparable<Num> {
	...
	/**
	 * Return number equal to "this" number, in base=newBase * @param newBase
	 * 
	...
	/**
	 * Evaluate an expression in infix and return resulting number Each string is
	 * one of: "*", "+", "-", "/", "%", "^", "(", ")", "0", or a number:
	...
	public static Num evaluateInfix(String[] expr) {
		...
		for (int i = 0; i < expr.length; i++) {
			...
			if (isOperator(cur)) {
				...
				while (!opStack.isEmpty() && opStack.peek() != "(" && (precedence(opStack.peek()) > precedence(cur)
						|| (precedence(opStack.peek()) == precedence(cur) && isLeftAssociative(opStack.peek())))) {
